...
class List extends Component {
  ...
  componentDidMount() {
    floatingButton.onClick = () => {
      pushStore.reset()
      this.props.history.push('/dashboard/push/new')
    }
    floatingButton.show()

    if (user.isLoggedIn) if (pushStore.pushMessages.length === 0) pushStore.fetchPushMessagess()
    ...
    return (
      <div className={styles.container} >
        <Table 
          ...
          onRowClick={async id => {
            this.props.history.push(`/dashboard/push/${id}`)
          }}
          onDelete={() => {
            if (this.selected.length === 0) return
            this.isDeleteDialogActive = true
          }}
          path="/api/push_messages"
          filterName="title"
  ...
  onDelete = async () => {
    try {
      ...
      for (let id of toBeDeleted) await pushStore.deletePushMessages(id)
      snackbar.show(`${toBeDeleted.length} messages deleted`)
      await this.refetch()
    } catch (err) {
  ...
  render() {
    ...
    return (
      <div className={styles.container} >
        {
          pushStore.isFetchingPushMessages
